    ...
    : input_size(input_size_), hidden_size(hidden_size_), output_size(output_size_), learning_rate(lr)
{
    ...
    std::uniform_real_distribution<double> dist(-1.0, 1.0);	//Gewichte zwischen -1 und 1
...
void SimpleNN::train(const std::vector<double>& input, int action, double target) {
    ...
    double error = (target - output_layer[action]) * (target - output_layer[action]);
    ...
    for (int j = 0; j < hidden_size; ++j) {
        ...
        d_hidden[j] = grad_sum * relu_derivative(hidden_layer[j]);  //"De"ReLu
    }
...
void SimpleNN::copyFrom(const SimpleNN& other) {
    W1 = other.W1;
    W2 = other.W2;
    b1 = other.b1;
    b2 = other.b2;
